...
int main() {
  ...
  vector<vector<double> >
    serialTimes(numberOfDataPoints,
                vector<double>(numbersOfThreads.size(), 0));
  vector<vector<double> >
    threadedTimes(numberOfDataPoints,
                  vector<double>(numbersOfThreads.size(), 0));
    ...
    for (unsigned int numberOfThreadsIndex = 0;
      ...
      try {
        ...
        runTest(numberOfTrials,
                ...
                &threadedTimes[dataPointIndex][numberOfThreadsIndex]);
  ...
  Utilities::writeMatrixToFile(threadedTimes, prefix + string("threaded") + suffix);
