...
colors      = cm.jet(numpy.linspace(1, 0, 6))

plt.figure(figsize=(12, 8))
ax = plt.subplot(111)
plt.xscale('log')
legendNames = []
plt.plot(data[:,0], data[:,1] / data[:,6], color=colors[0], hold='on', linewidth=2)
legendNames.append('gpu cublas')
plt.plot(data[:,0], data[:,1] / data[:,4], color=colors[4], hold='on', linewidth=2)
legendNames.append('gpu row st tiled mult shared')
...
plt.plot(data[:,0], data[:,1] / data[:,5], color=colors[3], hold='on', linewidth=2)
