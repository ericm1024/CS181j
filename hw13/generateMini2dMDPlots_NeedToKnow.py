  ...
  try:
    while (fileIndex < 10000):
      ...
      for rank in range(numberOfSimulationProcesses):
        filename = '%s%05d_%03d.csv' % (prefix, fileIndex, rank)
        try:
          ...
          if (numberOfLinesInThisFile > 2):
            ...
            pointColor = (int(colors[rank][0] * 255), int(colors[rank][1] * 255), int(colors[rank][2] * 255))
...
if __name__ == '__main__':
  ...
  prefix = 'data/Mini2dMD_NeedToKnow_%03d_' % (numberOfSimulationProcesses)
  outputPrefix = 'figures/Mini2dMD_NeedToKnow_%03d_' % (numberOfSimulationProcesses)
